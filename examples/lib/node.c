...
int debugNode, infoNode, placeNode, lineNode;
static int print(Node *p, FILE *fp, char *tab[], int lev) {
    ...
    if (lineNode) fprintf(fp," $%d", p->line);
    if (tab != 0)
...
int visitorNode(Node *p, int (*pre)(Node*, void*), int (*pos)(Node*, void*), void *u) {
    unsigned i;
    int cnt = 0;
    if (p == 0) return cnt;
    if (pre) cnt += pre(p, u);
    if (p->type == nodeOpr)
	for (i = 0; i < p->value.sub.num; i++)
	    visitorNode(p->value.sub.n[i], pre, pos, u);
    if (pos) cnt += pos(p, u);
    return cnt;
}
